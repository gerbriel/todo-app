...
-- FIRST: Insert your admin email BEFORE enabling RLS
INSERT INTO allowed_users (email, is_admin) 
VALUES ('gabrielriosemail@gmail.com', true)
ON CONFLICT (email) DO UPDATE SET is_admin = true;

-- Set up Row Level Security (RLS) AFTER inserting admin
...
DROP POLICY IF EXISTS "Only approved users can view profiles" ON profiles;
...
-- RLS Policies - Fixed to work properly
CREATE POLICY "Users can view their own profile" ON profiles FOR SELECT USING (auth.uid() = id);
CREATE POLICY "Users can update their own profile" ON profiles FOR UPDATE USING (auth.uid() = id);

-- Allow users to view other approved users (needed for app functionality)
CREATE POLICY "Approved users can view other approved users" ON profiles FOR SELECT USING (
  EXISTS (
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_checklists_updated_at') THEN
